...
SELECT DISTINCT q.business_id, q.name, q.full_address, t.stars, t.review_count, t.open
FROM q q LEFT OUTER JOIN t t ON (q.business_id = t.business_id)
...
SELECT DISTINCT r.business_id, s.latitude, s.longitude, r.city, r.state, s.neighborhood
FROM r r LEFT OUTER JOIN s s ON (r.business_id = s.business_id)
ORDER BY business_id
;
*/
